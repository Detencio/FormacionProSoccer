...
const PlayerCard: React.FC<PlayerCardProps> = React.memo(({
  ...
  const handleCardClick = useCallback((e: React.MouseEvent) => {
    e.preventDefault()
    e.stopPropagation()
    console.log('PlayerCard clicked:', player.name, 'selected:', isSelected)
    if (isSelected) {
  ...
  return (
    ...
    >
      {/* Header con color más claro y elegante */}
      <div className="bg-gradient-to-r from-slate-600 to-slate-500 p-4">
        <div className="flex items-center justify-between">
          <div className="flex-1 min-w-0">
            <h3 className="text-white font-semibold text-base truncate mb-2">{player.name}</h3>
            <div className="flex items-center space-x-3">
              <span className={`px-3 py-1 ${getPositionColor(position)} rounded-full text-xs font-bold text-white shadow-md`}>
                {position}
              </span>
              <span className="text-gray-200 text-sm font-medium">#{player.jersey_number || 'N/A'}</span>
            </div>
          </div>

          {/* Nivel de habilidad más prominente */}
          <div className="flex items-center justify-center ml-3">
            <div className="bg-white rounded-full w-8 h-8 flex items-center justify-center shadow-md border-2 border-white/30">
      ...
      {/* Body con estadísticas en una fila */}
      <div className="p-4">
        {/* 6 habilidades en una fila - descripción arriba, números abajo */}
        <div className="grid grid-cols-6 gap-2">
          <div className="text-center">
            <div className="text-xs text-gray-600 mb-1 font-medium">RIT</div>
            <div className={`text-sm font-bold ${getStatColor(stats.rit)}`}>{stats.rit}</div>
          </div>
          <div className="text-center">
            <div className="text-xs text-gray-600 mb-1 font-medium">TIR</div>
            <div className={`text-sm font-bold ${getStatColor(stats.tir)}`}>{stats.tir}</div>
          </div>
          <div className="text-center">
            <div className="text-xs text-gray-600 mb-1 font-medium">PAS</div>
            <div className={`text-sm font-bold ${getStatColor(stats.pas)}`}>{stats.pas}</div>
          </div>
          <div className="text-center">
            <div className="text-xs text-gray-600 mb-1 font-medium">REG</div>
            <div className={`text-sm font-bold ${getStatColor(stats.reg)}`}>{stats.reg}</div>
          </div>
          <div className="text-center">
            <div className="text-xs text-gray-600 mb-1 font-medium">DEF</div>
            <div className={`text-sm font-bold ${getStatColor(stats.defense)}`}>{stats.defense}</div>
          </div>
          <div className="text-center">
            <div className="text-xs text-gray-600 mb-1 font-medium">FIS</div>
      ...
      {/* Indicador de selección más prominente */}
      {isSelected && (
      ...
      {/* Badge de invitado más elegante */}
      {player.is_guest && (
